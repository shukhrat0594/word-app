...
| — | **IELTS bo'limi ruxsatlari: Talaba/Admin/Owner ko'radi, oddiy foydalanuvchi yo'q** (2026-07-20) — avval faqat markazga biriktirilgan foydalanuvchilar ko'rar edi (owner/admin markazi bo'lmasa ham ko'rishi kerak edi). Endi: `korinadigan_testlar()` admin/owner uchun barcha testlarni qaytaradi (markazidan qat'iy nazar), frontend filtri esa aniq `role === "oddiy"` bo'yicha yashiradi (markaz mavjudligiga qarab emas). Admin nav'iga talaba tomonidagi "/ielts" havolasi ham qo'shildi (avval faqat "IELTS testlari" boshqaruv sahifasi bor edi). Brauzerda admin va oddiy foydalanuvchi hisoblari bilan tekshirildi | **Bajarildi (2026-07-20)** |
