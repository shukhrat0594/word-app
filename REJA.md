...
| **B2.1** ⏳ | Guruh va Google OAuth | Talaba **Google OAuth** orqali ro'yxatdan o'tadi (Gmail akkount, parolsiz). Yangi **Guruh** modeli — nomi, Markaz FK, biriktirilgan **O'qituvchi** (admin tayinlaydi), **Talabalar** (admin guruhga qo'shadi). O'qituvchi/Admin akkountlari hamon admin tomonidan qo'lda yaratiladi (o'z-o'zidan ro'yxatdan o'tmaydi). `academics` app: `Guruh` modeli + `m2m_changed` signal (talaba guruhga qo'shilganda markazsiz bo'lsa avtomatik biriktiriladi, boshqa markazga tegishli bo'lsa xato beradi) — **shell orqali tekshirildi, ishlaydi**. `POST /api/auth/google/` endpoint (`google-auth` bilan ID token tekshiradi, JWT qaytaradi) — **kod yozildi, xato holatlar (token yo'q→400, yaroqsiz→401) tekshirildi**. To'liq oqim (haqiqiy Google Client ID bilan) — **Shuxrat Google Cloud Console'da OAuth Client ID yaratishi kerak**, keyin to'liq sinaladi | Talaba Google orqali kirib, admin uni guruhga va o'qituvchini guruhga biriktirganini tekshirish |
| **B2.2** | Davomat (soddalashtirilgan yo'qlama) | Dars jadvali/kalendar YO'Q — o'qituvchi guruh sahifasida kun bo'yicha **"kim keldi"** belgilaydi (Keldi/Kelmadi). Model: sana + guruh + talaba + holat. B6/B6.1'da ko'rsatiladi | O'qituvchi bir kunlik davomatni belgilab, saqlanganini va B6.1'da ko'rinishini tekshirish |
| **B3** | Kurslar va Kontent (Private/Public) | Kurs/Dars/Material modellari — matn, video, **audio (Listening uchun)**. Material **ixtiyoriy ravishda Darsga bog'lanadi** (video dars ichidagi audio/matn) **yoki mustaqil** bo'ladi (lug'at, lug'at o'yinlari). Material turi: Shaxsiy (Private) / Umumiy (Public) | Admin panelda material (jumladan audio dars, va Dars'siz mustaqil lug'at materiali) yaratib, turini tekshirish |
| **B3.1** | Umumiy kontent — ochilish sharti | Markaz **5 ta** Umumiy (Public) material kiritgandan keyin — boshqa markazlarning umumiy materiallarini **ko'rish huquqi** ochiladi (o'qituvchi/admin darajasida). **Xuddi shu 5 ta chegarada, bir vaqtda** — o'sha markaz **talabalari** ham umumiy materiallardan **foydalanish huquqi**ga ega bo'ladi (ikki bosqich alohida emas, bitta chegara) | 5-materialgacha yopiq, 5-dan keyin o'qituvchi/admin VA talabalar uchun bir vaqtda ochilishini tekshirish |
